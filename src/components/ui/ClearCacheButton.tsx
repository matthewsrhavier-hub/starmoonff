...
export function ClearCacheButton({
  ...
  const clearAllCache = async () => {
    ...
    try {
      // 1. Limpar localStorage SELETIVAMENTE (mantendo Auth)
      const keysToKeep = ['auth_token', 'token', 'user', 'sb-', 'supabase-auth'];
      ...
      // 3. Limpar cookies SELETIVAMENTE
      document.cookie.split(';').forEach((cookie) => {
        const eqPos = cookie.indexOf('=');
        const name = eqPos > -1 ? cookie.substring(0, eqPos).trim() : cookie.trim();
        // Nao remove cookies do Supabase ou o token de sessao
        if (!keysToKeep.some(k => name.toLowerCase().includes(k))) {
